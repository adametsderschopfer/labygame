...
void UMazeECSSubsystem::Initialize(FSubsystemCollectionBase& Collection)
{
	...
	const UScriptStruct* Fragments[] = { FMazeVitalsFragment::StaticStruct(), FMazeLocomotionFragment::StaticStruct(),
		FMazePlayerInputFragment::StaticStruct(), FMazePlayerPoseFragment::StaticStruct(),
		FMazePlayerCommandFragment::StaticStruct(), FMazeProgressFragment::StaticStruct() };
...
{
	check(IsInGameThread());
	if (!MassSubsystem) return FMassEntityHandle();
...
void UMazeECSSubsystem::SetInputAxis(FMassEntityHandle Entity, EMazeInputAxis Axis, float Value)
{
	...
	if (auto* Input = FindFragment<FMazePlayerInputFragment>(Entity))
	{
		switch (Axis)
		{
		case EMazeInputAxis::Forward: Input->Forward = FMath::Clamp(Value, -1.f, 1.f); break;
		case EMazeInputAxis::Right: Input->Right = FMath::Clamp(Value, -1.f, 1.f); break;
		case EMazeInputAxis::Yaw: Input->Yaw += Value; break;
		case EMazeInputAxis::Pitch: Input->Pitch += Value; break;
		}
	}
}
...
void UMazeECSSubsystem::SetInputAction(FMassEntityHandle Entity, EMazeInputAction Action, bool bPressed)
{
	if (auto* Input = FindFragment<FMazePlayerInputFragment>(Entity))
	{
		if (Action == EMazeInputAction::Sprint) Input->bSprintHeld = bPressed;
		else
		{
...
{
	if (!MassSubsystem || !InputQuery) return;
	auto Context = MassSubsystem->GetMutableEntityManager().CreateExecutionContext(0.f);
	InputQuery->ForEachEntityChunk(Context, [](FMassExecutionContext& Chunk)
	{
...
FMazePlayerCommandFragment UMazeECSSubsystem::ResolvePlayer(FMassEntityHandle Entity, const FMazePlayerPoseFragment& Pose)
{
...
void UMazeECSSubsystem::UpdateProgress(FMassEntityHandle Entity)
{
	...
	if (!Progress || !Pose || !Vitals || !FMazeVitalsSystem::IsAlive(*Vitals)) return;
	...
	if (const auto* Maze = FindFragment<FMazeGenerationFragment>(Progress->Maze))
	{
		...
		if (!Progress->ReachedExit) Progress->ReachedExit = FMazeGenerationSystem::ExitAt(*Maze, Pose->Location);
	}
}
...
{
	if (!MassSubsystem || !GenerationQuery) return;
	auto Context = MassSubsystem->GetMutableEntityManager().CreateExecutionContext(0.f);
	GenerationQuery->ForEachEntityChunk(Context, [](FMassExecutionContext& Chunk)
	{
		auto Mazes = Chunk.GetMutableFragmentView<FMazeGenerationFragment>();
		for (auto& Maze : Mazes) FMazeGenerationSystem::Generate(Maze);
	});
}
...
FMassEntityHandle UMazeECSSubsystem::CreateMaze(int32 Seed, FVector Origin)
{
	...
	if (auto* Session = FindFragment<FMazeSessionFragment>(SessionEntity)) Session->Maze = Entity;
	return Entity;
}
...
void UMazeECSSubsystem::RegenerateMaze(FMassEntityHandle Entity, int32 Seed, FVector Origin)
{
	...
	{
		if (Maze->Seed == Seed && Maze->Origin == Origin && Maze->Data) return;
		Maze->Seed = Seed;
		Maze->Origin = Origin;
...
{
	if (auto* Session = FindFragment<FMazeSessionFragment>(SessionEntity)) Session->bSessionStarted = bStarted;
}
