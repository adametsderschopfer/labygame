...
struct FMazeGenerationSystem
{
	...
	{
		if (!Maze.bNeedsGeneration) return;
		...
		Data->FloorTransform = FTransform(FRotator::ZeroRotator, FVector(Span / 2, Span / 2, -25),
			FVector((Span + 2400) / 100, (Span + 2400) / 100, 0.5f));
		Data->Start = FVector((Data->Layout.Size / 2 + 0.5f) * Maze.Cell, (Data->Layout.Size / 2 + 0.5f) * Maze.Cell, 100.f);
		...
		for (int32 I = 0; I < Data->Layout.Exits.Num(); ++I)
		{
			...
			Data->ExitPositions.Add(FVector((C % Data->Layout.Size + 0.5f) * Maze.Cell,
				(C / Data->Layout.Size + 0.5f) * Maze.Cell, 300.f) + Outward[I] * (Maze.Cell / 2 + 50));
	...
	static int32 ExitAt(const FMazeGenerationFragment& Maze, const FVector& Location)
	{
		...
		for (int32 I = 0; I < Layout.Exits.Num(); ++I)
		{
			...
			if ((I == 0 && P.Y < -50 && FMath::Abs(P.X - X) < HalfOpening) ||
				...
				(I == 2 && P.Y > Span + 50 && FMath::Abs(P.X - X) < HalfOpening)) return I + 1;
		}
		return 0;
	}
};
...
struct FMazePlayerControlSystem
{
	static FMazePlayerCommandFragment Resolve(FMazePlayerInputFragment& Input,
		const FMazePlayerPoseFragment& Pose, const FMazeVitals& Vitals, FMazeLocomotionFragment& Locomotion)
	{
		FMazePlayerCommandFragment Command;
		Command.bDead = !FMazeVitalsSystem::IsAlive(Vitals);
		if (!Pose.bInputEnabled) Input = FMazePlayerInputFragment();
		...
		Locomotion.bRunning = bSprint && Pose.bOnGround && Pose.Velocity.SizeSquared2D() > 1.f
			&& !Pose.Acceleration.IsNearlyZero();
		return Command;
	}
};
