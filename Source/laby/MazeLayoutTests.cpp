...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FMazeLayoutTest, "Laby.Maze.ConnectivityAndExits", EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
bool FMazeLayoutTest::RunTest(const FString& Parameters)
{
	for (int32 Seed = 1; Seed <= 100; ++Seed)
	{
		...
		TSet<int32> Seen;
		TArray<int32> Queue;
		Queue.Add(Maze.Start()); Seen.Add(Maze.Start());
		int32 Openings = 0;
		...
		for (int32 Head = 0; Head < Queue.Num(); ++Head)
		{
			...
			for (int32 D = 0; D < 4; ++D)
			{
				if (Maze.Walls[C] & (1 << D)) continue;
				int32 X = C % Maze.Size + DX[D], Y = C / Maze.Size + DY[D];
				if (X < 0 || Y < 0 || X >= Maze.Size || Y >= Maze.Size) { ++Openings; continue; }
				int32 N = Y * Maze.Size + X;
				if (Maze.Walls[N] & (1 << ((D + 2) % 4))) { AddError(TEXT("Asymmetric wall")); return false; }
		...
		for (int32 Exit : Maze.Exits) TestTrue(TEXT("Exit reachable"), Seen.Contains(Exit));
