...
void AMazeHUD::DrawHUD()
{
	Super::DrawHUD();
	if (!Canvas) return;
	if (const auto* PC = Cast<AMazePlayerController>(PlayerOwner); PC && PC->IsMenuOpen()) return;
	...
	if (const auto* Character = PlayerOwner ? Cast<AMazeCharacter>(PlayerOwner->GetPawn()) : nullptr)
	{
		...
		if (!FMazeVitalsSystem::IsAlive(Vitals))
		{
			...
			DrawText(TEXT("Press R to start a new maze"), FLinearColor::White, Canvas->ClipX / 2 - 180, Canvas->ClipY / 2 + 4);
			return;
		}
	}
	...
	if (ReachedExit)
	{
		...
		DrawText(FString::Printf(TEXT("EXIT %d REACHED"), ReachedExit), FLinearColor(0.3f,1.f,0.6f), Canvas->ClipX / 2 - 180, Canvas->ClipY / 2 - 50, nullptr, 2);
		DrawText(TEXT("Press R to generate a new maze"), FLinearColor::White, Canvas->ClipX / 2 - 180, Canvas->ClipY / 2 + 4);
	}
}
...
void AMazeHUD::DrawMinimap(const AMazeWorld& Maze)
{
	...
	for (int32 Y = 0; Y < Layout.Size; ++Y)
		for (int32 X = 0; X < Layout.Size; ++X)
		{
			...
			if (W & 1) DrawLine(PX, PY, PX + Step, PY, WallColor);
			if (W & 8) DrawLine(PX, PY, PX, PY + Step, WallColor);
			if (X == Layout.Size - 1 && (W & 2)) DrawLine(PX + Step, PY, PX + Step, PY + Step, WallColor);
			if (Y == Layout.Size - 1 && (W & 4)) DrawLine(PX, PY + Step, PX + Step, PY + Step, WallColor);
		}
