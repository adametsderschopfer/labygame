	...
	void Build(const FMazeLayout& Layout, float Cell, float Thickness, float Height)
	{
		...
		for (int32 Y = 0; Y < N; ++Y)
			for (int32 X = 0; X < N; ++X)
			{
				...
				if (!Occupied(X, Y - 1))
					Face({A, C, 0}, {B, C, 0}, {B, C, Height}, {A, C, Height}, {0, -1, 0});

				if (!Occupied(X, Y + 1))
					Face({B, D, 0}, {A, D, 0}, {A, D, Height}, {B, D, Height}, {0, 1, 0});

				if (!Occupied(X - 1, Y))
					Face({A, D, 0}, {A, C, 0}, {A, C, Height}, {A, D, Height}, {-1, 0, 0});

				if (!Occupied(X + 1, Y))
					Face({B, C, 0}, {B, D, 0}, {B, D, Height}, {B, C, Height}, {1, 0, 0});
			}
	}
};
