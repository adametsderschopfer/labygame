...
void AMazePlayerController::ShowMenu(bool Settings)
{
	...
	{
		Items->AddSlot().AutoHeight().Padding(0, 0, 0, 18)[SNew(STextBlock)
	...
	{
		Items->AddSlot().AutoHeight().Padding(0,
		                                      5)[SNew(SButton)
		                     ...
		                     .Text_Lambda(
		                         []()
		                         {
			                         return FText::FromString(FString::Printf(
			                             TEXT("%.2f ×"), GetDefault<UMazePreferences>()->GetSensitivity()));
		                         })
		                     .Font(FCoreStyle::GetDefaultFontStyle("Regular", 22))];
		Items->AddSlot().AutoHeight().Padding(
		    0, 8, 0, 24)[SNew(SSlider)
		                     .MinValue(0.1f)
		                     .MaxValue(3.f)
		                     .StepSize(0.05f)
		                     .Value_Lambda(
		                     ...
		                     .OnMouseCaptureEnd_Lambda(
		                         []()
		                         {
			                         auto* P = GetMutableDefault<UMazePreferences>();
			                         P->SetSensitivity(P->GetSensitivity());
		                         })
	...
	{
		Text(ReadSession().bSessionStarted ? TEXT("Игра приостановлена") : TEXT("Один лабиринт. Три выхода."),
		     18,
		...
		Button(TEXT("Выйти на рабочий стол"),
		       [this]()
		       {
	...
	TSharedRef<SMazeMenuRoot> Root =
	    SNew(SMazeMenuRoot)[SNew(SBorder)
	                            .BorderImage(FCoreStyle::Get().GetBrush("WhiteBrush"))
	                            .BorderBackgroundColor(FLinearColor(0.015f, 0.025f, 0.04f, 1))
	                            .HAlign(HAlign_Center)
	                            .VAlign(VAlign_Center)[SNew(SScaleBox).Stretch(
	                                EStretch::ScaleToFit)[SNew(SBox).WidthOverride(460).Padding(30)[Items]]]];
