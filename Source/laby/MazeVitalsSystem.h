...
{
	static bool IsAlive(const FMazeVitals& V) { return V.Health > 0.f; }
	static bool CanSprint(const FMazeVitals& V) { return IsAlive(V) && !V.bExhausted && V.Stamina > 0.f; }
	static bool CanJump(const FMazeVitals& V) { return IsAlive(V) && V.Stamina >= FMazeVitals::JumpCost; }
	static bool SpendJumpStamina(FMazeVitals& V)
	{
		...
		if (V.Stamina <= 0.f) V.bExhausted = true;
		return true;
	}
	static void Update(FMazeVitals& V, float DeltaSeconds, bool bRunning, bool bOnGround = true)
	{
		if (!IsAlive(V) || !FMath::IsFinite(DeltaSeconds) || DeltaSeconds <= 0.f) return;
		...
		else
		{
			...
			if (V.Stamina >= FMazeVitals::ResumeThreshold) V.bExhausted = false;
		}
	}
	static float Damage(FMazeVitals& V, float Amount)
	{
		if (!FMath::IsFinite(Amount) || Amount <= 0.f || !IsAlive(V)) return 0.f;
